...
import { FaTimes, FaUserNurse, FaPlaneDeparture, FaListOl, FaUserMd, FaUserAlt, FaHandHoldingHeart} from "react-icons/fa"; // Import icons
import {
  Accordion,
  AccordionContent,
  AccordionItem,
...
const KeyServices: React.FC<SidebarProps> = ({ isOpen, toggleSidebar }) => {
  ...
  return (
    <div
      ref={sidebarRef}
      className="fixed right-0 bg-white shadow-xl w-[60rem] z-50 transition-transform duration-300"
      style={{ top: "240px", height: "55vh" }}
    >
      {/* Close button */}
      <button
        onClick={toggleSidebar}
        className="bg-blue-500 text-white z-20 absolute left-[-3.5rem] top-4"
        style={{
          fontSize: "2rem",
